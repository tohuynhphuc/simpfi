...
public class InspectPanel extends Panel {
	...
	/**
	 * Opens a dialog that allows the user to filter vehicles for export. The dialog
	 * provides options to filter by vehicle type (big, small, private, commercial),
	 * active status, traveled distance and congestion threshold. Users can then
	 * export the filtered vehicles as CSV or PDF.
	 */
	...
	private void exportFiltered(boolean csv, // true CSV, false PDF
		...
		List<Vehicle> filtered = getFilteredVehicles(bigVehiclesCheck.isSelected(), smallVehiclesCheck.isSelected(),
			...
			parseDouble(minDistanceField.getText(), 0), parseDouble(maxDistanceField.getText(), Double.MAX_VALUE),
			congestedEdgesCheck.isSelected(), threshold);
	...
	private List<Vehicle> getFilteredVehicles(boolean includeBig, boolean includeSmall, boolean includePrivate,
		...
		return snapshot.stream().filter(v -> {
	...
	/**
	 * Parses a string into a double. If parsing fails, returns the provided
	 * fallback value.
	 *
	 ...
	 * @param threshold Minimum number of vehicles on the edge to consider it
	 *                  congested
