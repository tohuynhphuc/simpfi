 ...
 */
// Confirm changes doesnt work yet, because it only changes speed, which cant be
// changed due to predefined routes
// of vehicles
...
public class InspectPanel extends Panel {
	...
	public InspectPanel(SumoConnectionManager conn, MapPanel mapPanel) {
		...
		TextBox speedTextbox = createTextboxWithLabel("Speed", statsScrollPane, 0.0, true, true,
		...
		statsWrapper.setLayout(new FlowLayout(FlowLayout.LEFT));
		...
		mapPanel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Point world = (new Point(e.getX(), e.getY())).fromMapToWorld();
	...
	private void confirmStats() {
		...
		String value = vehicleListModel.get(listIndex);
		if (value.startsWith("---")) {
			JOptionPane.showMessageDialog(this, "Please select a valid vehicle, not a header.", "Warning",
		...
		try {
			...
			// Speed set
			vehicleController.setSpeed(vehicleID, newSpeed);

			JOptionPane.showMessageDialog(this, "Vehicle speed updated successfully.", "Info",
				JOptionPane.INFORMATION_MESSAGE);

		} catch (NumberFormatException ex) {
			JOptionPane.showMessageDialog(this, "Invalid number format!", "Error", JOptionPane.ERROR_MESSAGE);
		} catch (Exception ex) {
			JOptionPane.showMessageDialog(this, "Failed to update vehicle in SUMO: " + ex.getMessage(), "Error",
