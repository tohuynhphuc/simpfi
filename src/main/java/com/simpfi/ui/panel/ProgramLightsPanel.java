...
public class ProgramLightsPanel extends Panel {
	...
	private void generateDropdowns(String[] allTLJunctionIDs, String firstJunctionID) throws Exception {
		...
		// Add a listener to update highlighted connection when connection dropdown
		// changes
	...
	// Settings.highlight.HIGHLIGHTED_TRAFFIC_LIGHT = TrafficLight
	// .searchforTrafficLight((String) tlJunctionDropDown.getSelectedItem(),
	...
	/**
	 * Get all the connection string and show it to the drop down as a string
	 * builder.
	 * 
	...
	public String[] getAllStringConnection(List<Connection> allConnections) {
		...
		for (int i = 0; i < allConnections.size(); i++) {
			...
			// System.out.println(allConnections.get(i).getFromLane().getLaneId() + " " +
			// allConnections.get(i).getToLane().getLaneId());
	...
	public void showInformationTrafficLightToDialog(String trafficLightID) throws Exception {
		...
		for (Connection c : allConnections) {
			...
			sb.append("From: ").append(fromLaneString).append(" → To: ").append(toLaneString).append(" | Signal: ")
	...
	public void applyChangeDuration(String trafficLightID, int phaseNumber, double duration) throws Exception {
		// In the future I will make a class get duration with parameter phaseNumber and
		// trafficLightID
		SumoTLSProgram program = trafficLightController.getProgramFromTrafficLight(trafficLightID);
	...
	public void actionForChangingMode() {
		if (changingAdaptiveModeOrStaticMode.getText().equals("Change to Static Mode")) {
			...
			adaptiveMode = new TrafficMetric(stats.getAverageSpeed(), stats.getCongestedEdges(5.0).size(),
			...
			staticMode = new TrafficMetric(stats.getAverageSpeed(), stats.getCongestedEdges(5.0).size(),
				calculateAverageTimeTravel(stats.getTravelTimesArray()));
		}
	}
	...
	private void showComparison(TrafficMetric adaptiveMode, TrafficMetric staticMode) {
		...
		Object[][] data = {
			{ "Average Speed (m/s)", String.format("%.2f", staticMode.getAvgSpeed()),
				String.format("%.2f", adaptiveMode.getAvgSpeed()),
				String.format("%.2f", Math.abs(adaptiveMode.getAvgSpeed() - staticMode.getAvgSpeed())) },

			{ "Congested Edges", staticMode.getTotalCongestion(), adaptiveMode.getTotalCongestion(),
				Math.abs(adaptiveMode.getTotalCongestion() - staticMode.getTotalCongestion()) },

			{ "Average Time Travel", String.format("%.2f", staticMode.getAvgTravelTime()),
	 ...
	 * in a visually grouped and readable format. The text area is wrapped inside a
	 * scroll pane to support variable-length content.
	 * </p>
	 *
	 ...
	 * </p>
	 * <p>
	 * UI updates are executed on the Swing Event Dispatch Thread (EDT) to ensure
	 * thread safety.
	 * </p>
	 */
	 ...
	 * <p>
	 * This method formats simulation performance metrics into a human-readable text
	 * block and updates the associated {@link TextArea}.
	 * </p>
	 * <p>
	 * The update is dispatched on the Swing Event Dispatch Thread (EDT) to maintain
	 * Swing thread safety.
	 * </p>
	 *
	...
	public void updateImpactDisplay(double avgSpeed, int congestedCount, double avgTravelTime) {
		String feedback = "Impact of Timing Adjustment:\n" + "Average Speed: " + String.format("%.2f m/s", avgSpeed)
			+ "\n" + "Congested Edges: " + congestedCount + "\n" + "Average Travel Time: "
			+ String.format("%.2f sec", avgTravelTime);
	 ...
	 * <p>
	 * This method creates a non-editable, word-wrapped {@link TextArea}, embeds it
	 * into a formatted card using {@code createCard}, and attaches the card to the
	 * parent panel.
	 * </p>
	 * <p>
