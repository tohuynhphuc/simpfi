...
public class VehiclePdfExporter {
	 ...
	 * @param congestionThreshold Threshold defining when an edge is considered
	 *                            congested
	 */
	...
	/**
	 * Creates a simple horizontal bar chart showing congested edges. Bar color
	 * indicates congestion: green = low, yellow = medium, red = high.
	 *
	 ...
	 */
	private static Image createCongestionChart(PdfWriter writer, List<Vehicle> vehicles, int threshold)
		throws Exception {
		...
		vehicles.stream().map(Vehicle::getEdgeFromRoadID).filter(edge -> edge != null).distinct().forEach(edge -> {
			long count = vehicles.stream()
				.filter(v -> v.getEdgeFromRoadID() != null && v.getEdgeFromRoadID().getId().equals(edge.getId()))
				.count();
			if (count >= threshold) {
		...
		JFreeChart chart = ChartFactory.createBarChart("Congested Edges", "Edge ID", "Vehicle Count", dataset,
